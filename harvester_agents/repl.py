...
async def run_demo_loop(
    ...
    Args:
        agent: Agent instance (GrokCodeAgent, ClaudeCodeAgent, or raw Agent from agents SDK)
    ...
    # Get raw agent
    raw_agent = agent.agent if hasattr(agent, 'agent') else agent
...
async def run_interactive_chat(
    ...
    Args:
        agent: Agent instance (GrokCodeAgent, ClaudeCodeAgent, etc.)
    ...
    session = None
    if session_id and AgentSession:
...
async def main():
    ...
    import argparse
    from .grok_code_agent import GrokCodeAgent

    parser = argparse.ArgumentParser(description="Interactive agent REPL")
    parser.add_argument("--model", default="grok-code-fast-1", help="Model to use")
